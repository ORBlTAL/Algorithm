...
# 목표 지점들의 좌표 리스트
target_points = [(1, 2), (-2, -3), (3, 1), (-1, 4)] # 실제 시험때는 몇번째 공을 먼저 칠 지 모르니 print로 디버깅 해가야 함
# 목적구가 4개라 가정하고, 2번째로 가까운 공을 친다고 가정
...
for point_x, point_y in target_points:
    ...
    # math.atan은 쓸 필요 없음. 1,2,3,4 분면 모두 구분이 가능한 atan2만 숙지하면 된다.
    # 2. 각도 계산 (atan2 함수 활용)
    # atan2(y, x) 순서로 인자를 전달하여 정확한 각도(라디안) 계산
    angle_radians = math.atan2(point_y - origin_y, point_x - origin_x) # 이 부분만 atan과 사용법이 다르다
